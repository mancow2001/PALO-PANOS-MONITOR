#!/usr/bin/env python3
"""
Updated Interface Monitoring for Your Specific PAN-OS 11 System
Optimized based on actual debug test results showing working interface commands
"""
import time
import logging
...
def parse_session_statistics_your_panos11(xml_text: str) -> Optional[SessionStats]:
    ...
    try:
        ...
        num_active = safe_session_extract(result_elem, [
            "num-active", "active-sessions", "num_active", "active", "sessions-active"
        ])
        
        num_max = safe_session_extract(result_elem, [
            "num-max", "max-sessions", "num_max", "maximum", "sessions-max"
        ])
        
        # Get detailed session counts if available
        tcp_sessions = safe_session_extract(result_elem, [
            "tcp-sessions", "tcp", "num-tcp", "sessions-tcp"
        ])
        
        udp_sessions = safe_session_extract(result_elem, [
            "udp-sessions", "udp", "num-udp", "sessions-udp"
        ])
        
        icmp_sessions = safe_session_extract(result_elem, [
            "icmp-sessions", "icmp", "num-icmp", "sessions-icmp"
        ])
        ...
        LOG.debug(f"Session parsing: active={num_active}, max={num_max}, tcp={tcp_sessions}, udp={udp_sessions}, icmp={icmp_sessions}")
...
def calculate_interface_metrics(prev_sample: InterfaceSample,
    ...
    LOG.debug(f"Calculated metrics for {curr_sample.interface_name}: "
...
class InterfaceMonitor:
    """Interface and session monitoring optimized for your PAN-OS 11 system"""
    ...
    def _check_authentication(self) -> bool:
        """Check if we're properly authenticated with the firewall"""
        try:
            # Simple test using a command we know works from debug results
    ...
    def start_monitoring(self):
        ...
        self.thread.start()
        LOG.info(f"{self.name}: Started interface monitoring for your PAN-OS 11")
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if (self.last_auth_check is None or
                    (current_time - self.last_auth_check).total_seconds() > 300):  # Check every 5 minutes
                    if not self._check_authentication():
                        consecutive_failures += 1
                        if consecutive_failures >= max_failures:
                            LOG.error(f"{self.name}: Too many authentication failures, stopping interface monitoring")
                            break
                        else:
                            LOG.warning(f"{self.name}: Authentication failed ({consecutive_failures}/{max_failures})")
                    else:
                        consecutive_failures = 0
    ...
    def _collect_interface_stats(self) -> bool:
        """Collect interface statistics using two-stage collection method"""
        try:
            LOG.info(f"{self.name}: Collecting interface statistics using two-stage method...")
            
            # Use the working two-stage collection method
            ...
            LOG.info(f"{self.name}: Successfully collected {len(current_samples)} interface samples")
    ...
    def _collect_session_stats(self) -> bool:
        """Collect session statistics using the working command for your system"""
        try:
            LOG.debug(f"{self.name}: Collecting session statistics...")
            
            # Use the session command that works on your system
            xml = self.client.op("<show><session><info/></session></show>")
            if not xml:
...
# Maintain backward compatibility - provide aliases with the old names
parse_interface_statistics_panos11 = parse_interface_statistics_your_panos11
parse_session_statistics_panos11 = parse_session_statistics_your_panos11

# Also provide the standard names for flexibility
parse_interface_statistics = parse_interface_statistics_your_panos11
parse_session_statistics = parse_session_statistics_your_panos11
